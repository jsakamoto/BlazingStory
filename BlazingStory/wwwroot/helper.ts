    ...
    window.addEventListener(
        ...
        (event) => {
            ...
            switch (message.action) {
                ...
                // This is required to ensure pop-up menus are closed, even when a user clicks inside an iframe.
                // (See also: BlazingStory/Internals/Pages/IFrame.razor.ts)
                case pointerdown:
                ...
                case 'frameview-height':
                    const previewFrameViewPort = doc.querySelector(
                        `.preview-frame-viewport:has(iframe#${message.frameId})`,
                    ) as HTMLElement | null;
