...
const iframesInPool: IFrameInPool[] & { remove: (iframe: HTMLIFrameElement) => void } = Object.assign([], {
    remove: (iframe: HTMLIFrameElement) => {
        const index = iframesInPool.findIndex(item => item.iframe === iframe);
        if (index >= 0) iframesInPool.splice(index, 1);
    }
});
...
const waitForIFrameReady = async <T>(iframe: HTMLIFrameElement) => {
    return await waitFor({
        predecate: () => {
            ...
            if (!iframe.contentWindow.Blazor) return false;
...
export const release = (containerElement: HTMLElement) => {
...
export const navigate = async (containerOrIFrame: HTMLElement | HTMLIFrameElement, url: string, baseUri: string): Promise<void> => {
    const iframe = (containerOrIFrame.tagName !== "IFRAME" ? containerOrIFrame.querySelector("iframe") : containerOrIFrame) as HTMLIFrameElement | null;
