...
export const navigatePreviewFrameTo = async (
...
export const zoomInPreviewFrame = (iframe: HTMLIFrameElement | null) =>
    zoomPreviewFrame(iframe, (zoom) => zoom * 1.25);

export const zoomOutPreviewFrame = (iframe: HTMLIFrameElement | null) =>
    zoomPreviewFrame(iframe, (zoom) => zoom / 1.25);

export const resetZoomPreviewFrame = (iframe: HTMLIFrameElement | null) =>
    zoomPreviewFrame(iframe, (_) => 1);

export const subscribeComponentActionEvent = async (
    iframe: HTMLIFrameElement | null,
    dotNetObj: DotNetObjectReference,
    methodName: string,
) => {
    try {
        if (iframe === null) return { dispose: () => { } };
        const { contentDocument } = await waitForIFrameReady(iframe);
        const componentActionEventListener = (e: Event & { detail?: any }) =>
            dotNetObj.invokeMethodAsync(methodName, e.detail.name, e.detail.argsJson);
        contentDocument.addEventListener(
            'componentActionEvent',
            componentActionEventListener,
        );
