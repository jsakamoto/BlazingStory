...
// tests/stories.json is written ahead of time by scripts/gen-stories.ts (from
// Playwright's globalSetup). Reading it synchronously here lets each story be
// registered as its own test() at collection time, which is what makes the
// stories show up individually in the VS Code Test Explorer. (Fetching the
// index directly is not an option: it needs a running browser, while test
// collection is synchronous.)
function loadStories(): StoryIndexEntry[] {
  try {
    ...
    // stories.json hasn't been generated yet; globalSetup writes it on the
    // first test run (the app must be running).
    return [];
  }
}
