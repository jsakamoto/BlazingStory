...
- The baselines live in `tests/vrt.spec.ts-snapshots/`: that is what `snapshots:pull` downloads into and what `snapshots:push` uploads from. Each file is named `<story id>-<platform>.png`, so a screenshot is only ever compared against a baseline captured on the same platform, and one container can hold the linux / win32 / darwin sets side by side.
