...
internal class StoriesTool
{
    ...
    [Description("""
        ...
        """)]
    public async Task<string> GetComponentParametersAsync(
        ...
        string componentName)
    {
        ...
        if (container is null) return $"Component '{componentName}' not found or parameter info not available.";

        var story = container.Stories.FirstOrDefault();
        if (story is null) return $"Parameter info of the component '{componentName}' not available since it has no stories.";
        ...
        var parameters = story.Context.Parameters.Select(p =>
        {
            ...
            {
                p.Name,
                Type = typeStrings.FirstOrDefault(),
                p.Required,
    ...
    [Description("""
        ...
        """)]
    public async Task<string> GetComponentStoriesAsync(
        ...
        string componentName)
    {
        ...
        var storiesStore = await this.BuildStoriesStoreAsync(scope);
        var container = storiesStore.StoryContainers.FirstOrDefault(c => c.TargetComponentType.Name == componentName);
        if (container is null) return $"Component '{componentName}' not found or has no stories.";
        ...
        var json = JsonSerializer.Serialize(stories, new JsonSerializerOptions
        {
            WriteIndented = true,
            PropertyNamingPolicy = JsonNamingPolicy.CamelCase
        });
